...
class DBConnection:
    ...
    def select(self, table: str, condition: list = [], order: dict = {}) -> tuple:
        '''
        select(table='test')\n
        select(table='test', order={'id1': 1})\n
        select(table='test', condition=["id1", "id2"])\n
        select(table='test', condition=["id1"], order={'id1': '1', 'id2': 2})\n
        :param table:
    ...
    def update(self, table: str, condition: list = [], values: list = [], order: list = [()]) -> bool:
        '''
        update(table="test", condition=["id1", "id2"], values=["val1", 1], order=[{"id1": "id", "id2": 123}])
        :param table:
        :param condition: default = []
        :param values: default = []
        :param order: default = [()]
